...
router.post("/add", (req, res) => {
    const { username, content, tags = [], color = "#fff", pinned = false } = req.body;
...
// List (gibt alle Notizen des Users zurück)
router.post("/list", (req, res) => {
    const { username } = req.body;
    if (!username) return res.status(400).json([]);
    const notes = loadNotes().filter(n => n.username === username);
    // Pinned zuerst, dann by created desc
    notes.sort((a,b) => {
...
// Edit (content, tags, color, pinned)
